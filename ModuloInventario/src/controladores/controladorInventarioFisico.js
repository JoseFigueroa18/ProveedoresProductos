    ...
    const listaModulos = [
        { 
            ...
            rutas: [
                ...
                {   
                    ruta: "/modulo/InventarioFisico/filtro",
                    metodo: "get",
                    parametros: {
                        filtro: "id del Inventario perteneciente al inventario fisico. Obligatorio."
                    },
                    descripcion: "Lista los datos con el inventario"
                },
                {   
...
exports.BuscarFiltro = async (req, res)=>{
    ...
    if (msj.errores.length > 0){
        ...
        try {
            ...
            filtro= '%'+filtro+'%';
            const buscarInventarioFisico = await InventarioFisico.findAll({
                attributes: ['id','productos_Codigo','inventarios_id','cantidadactual', 'cantidadsistema','costo','precio','fechahora'],
                include: {model: Producto,attributes:['Nombre']},
                where:{inventarios_id: filtro}
            });
            if(!buscarInventarioFisico){
                msj.estado = 'precaucion';
                msj.mensaje = 'La peticion se ejecuto correctamente';
                msj.errores={
                    mensaje: 'No existen inventarios con estos parametros',
                    parametro: 'filtro',
                };
            }
            else{
                msj.datos=buscarInventarioFisico;
            }
            msjRes(res, 200, msj);
        } catch (error) {
            msj.estado = 'error';
    ...
    if (msj.errores.length > 0){
        ...
        try {
            ...
            if (lista.length==0){
                ...
                msj.mensaje = 'La peticion se ejecuto correctamente';
                msj.errores={
                    mensaje: 'No existe ningun regitro con esta informacion',
                    parametro: 'filtro',
                };
            }
            else{
